		  ...
		  <table id="example1" class="table table-bordered table-striped">
			<thead>
			  <tr>
				...
				<th>Order Id <i class="fa fa-sort pull-right sortstyle" aria-hidden="true"></i></th>
				<th>Customer name <i class="fa fa-sort pull-right sortstyle" aria-hidden="true"></i></th>
				<th>Sender Name <i class="fa fa-sort pull-right sortstyle" aria-hidden="true"></i></th>
				<th>Order Status<i class="fa fa-sort pull-right sortstyle" aria-hidden="true"></i></th>
				<th>Produtcs<i class=" pull-right sortstyle" aria-hidden="true"></i></th>
			  </tr>
			</thead>
			<tbody>
			<?php
	  ...
	  <forM action="<?=base_url()?>orders" method="post">
		<div class="form-group">
			<select name="trackstatus" class="form-control">
				<?php $sql = 'Select * from order_track_status ';
				$query = $this->db->query($sql);
				foreach($row = $query->result() as $resOPT){
				?>	
				<option value="<?=$resOPT->statusId?>"><?=$resOPT->trackstatus?></option>
				<?php	
				}
			?>		
			</select>
		</div>
		<input type="hidden" id="odrField" name="odeID" value="">
