...
//        $where = array();
//        $query = $this->db->select('orders.*,customers.*,reciver.*,order_track.*,order_detail.*')
//                ->from('orders')
...
//                ->join('order_detail', 'orders.serial = order_detail.orderid')
//                ->where($where)
//                ->get();

        $query = $this->db->select('*')
                ->from('orders')
                ->join('users', 'orders.user_id = users.user_pk_id')
                ->get()
                ->result_array();
        return $result = $query;
    }
    ...
    function add_items($data) {
        $Q = $this->db->insert_batch('order_detail', $data);
        return $Q;
    }
